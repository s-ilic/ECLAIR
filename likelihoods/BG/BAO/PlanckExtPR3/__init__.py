...
class likelihood:
  def __init__(self, lkl_input):
    ...
    self.DR12_z = np.array([0.38, 0.38, 0.51, 0.51, 0.61, 0.61])
    self.DR12_data = np.array([1512.39, 81.2087, 1975.22, 90.9029, 2306.68, 98.9647])
    self.DR12_rsdrag_fid = 147.78
    ...
    self.DR12_icov_mat = np.linalg.inv(DR12_cov_mat)
  ...
  def get_loglike(self, class_input, lkl_input, class_run):
    ...
    theo = np.zeros(6)
    for i in range(3):
        DMz = class_run.angular_distance(self.DR12_z[i*2]) * (1. + self.DR12_z[i*2])
        Hz = class_run.Hubble(self.DR12_z[i*2]) * 299792.458
        theo[i*2] = DMz / rs * self.DR12_rsdrag_fid
        theo[i*2+1] = Hz * rs / self.DR12_rsdrag_fid
    diff = self.DR12_data - theo
    lnl += -0.5 * np.dot(np.dot(diff, self.DR12_icov_mat), diff)
    # Return log(like)
    return lnl
