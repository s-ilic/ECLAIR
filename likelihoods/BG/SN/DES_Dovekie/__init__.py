...
# from https://arxiv.org/abs/2401.02929

class likelihood:
    def __init__(self, lkl_input):
        ### Path to DESY5 likelihood folder (i.e. the folder this file is in)
        path_to_data = os.path.dirname(os.path.realpath(sys.argv[0]))
        path_to_data += '/likelihoods/BG/SN/DES_Dovekie/'
        data_file = path_to_data + "DES-Dovekie_HD.csv"
        inv_covmat_file =  path_to_data + "STAT+SYS.npz"
        self.twoscriptmfit = False
        self._read_data_file(data_file)
        self.covs = {}
        for name in ["mag"]:
            self.invcov = self._read_inv_covmat(inv_covmat_file)
    ...
    def _read_cols(self, data_file, file_cols, sep=None):
        with open(data_file, 'r') as f:
            lines = f.readlines()[8:]  # Skip the first 8 lines of header
            line = lines[0]
            if line.startswith('#'):
                line = line[1:]
