...
def compute_chi2(dlth, params_values, icov, lr):
    """
    ...
    delta_cl = np.asarray(Xl).astype('float32')[lr]

    # chi2 = self.delta_cl @ self._invkll @ self.delta_cl
    chi2 = icov.dot(delta_cl).dot(delta_cl)
...
class likelihood:
    def __init__(self, lkl_input):
        ...
        self.keep_ix_ell_range = []
        for ix, (l, m) in enumerate(all_ell_range):
            if int(lkl_input[f'hillipop_TTTEEE_ell_min_{m}']) <= l <= int(lkl_input[f'hillipop_TTTEEE_ell_max_{m}']):
                self.keep_ix_ell_range.append(ix)
        # read cov mat
        partial_icov_fname = "data/icov_PR4_v4.2_TTTEEE"
        partial_icov_fname += f"_TT-{int(lkl_input['hillipop_TTTEEE_ell_min_TT'])}-{int(lkl_input['hillipop_TTTEEE_ell_max_TT'])}"
        partial_icov_fname += f"_EE-{int(lkl_input['hillipop_TTTEEE_ell_min_EE'])}-{int(lkl_input['hillipop_TTTEEE_ell_max_EE'])}"
        partial_icov_fname += f"_TE-{int(lkl_input['hillipop_TTTEEE_ell_min_TE'])}-{int(lkl_input['hillipop_TTTEEE_ell_max_TE'])}"
        partial_icov_fname += ".npy"
        partial_icov_fname = os.path.join(data_folder, partial_icov_fname)
        if not os.path.isfile(partial_icov_fname):
            cut_covmat.create_cut_covmat(lkl_input)
        self._invkll = np.load(partial_icov_fname)
    ...
    def get_loglike(self, class_input, likes_input, class_run):
        ...
        # print(self.all_ell_range)
        chi2 = compute_chi2(dlth, likes_input, self._invkll, self.keep_ix_ell_range)
